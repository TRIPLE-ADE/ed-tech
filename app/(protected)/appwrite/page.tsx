...
import { DocumentStorageService } from "@/features/documents/services";
...
const AppwriteAuthUpload = () => {
  ...
  const loadFiles = async () => {
    try {
      setLoading(true);
      const response = await DocumentStorageService.listFiles({
        bucketId: BUCKET_ID,
      });
  ...
  const handleFileUpload = async (
    ...
    try {
      const response = await DocumentStorageService.uploadFile(file, {
        bucketId: BUCKET_ID,
        onProgress: (progress) => {
  ...
  const handleFileDownload = (fileId: string, fileName: string) => {
    const downloadUrl = DocumentStorageService.getFileDownload(
      BUCKET_ID,
      fileId
    );
  ...
  const handleFileDelete = async (fileId: string) => {
    try {
      await DocumentStorageService.deleteFile(BUCKET_ID, fileId);
      await loadFiles();
    } catch (error: any) {
      setError(error.message);
    }
  ...
  const formatFileSize = (bytes: number) => {
    return DocumentStorageService.formatFileSize(bytes);
  };
